...
type sentryGinHandler struct {
	repanic         bool
	waitForDelivery bool
	timeout         time.Duration
}
type SentryGinOptions struct {
	// Repanic configures whether Sentry should repanic after recovery, in most cases it should be set to true,
	// as gin.Default includes it's own Recovery middleware what handles http responses.
	Repanic bool
	// WaitForDelivery configures whether you want to block the request before moving forward with the response.
	// Because Gin's default Recovery sentryGinHandler doesn't restart the application,
...
func SentryGinNew(options SentryGinOptions) gin.HandlerFunc {
	timeout := options.Timeout
	if timeout == 0 {
		timeout = 2 * time.Second
	}
	return (&sentryGinHandler{
...
func (h *sentryGinHandler) handle(c *gin.Context) {
	...
	c.Set(valuesKey, hub)
	defer h.recoverWithSentry(hub, c)
	c.Next()
}

func (h *sentryGinHandler) recoverWithSentry(hub *sentry.Hub, c *gin.Context) {
	if err := recover(); err != nil {
		if !isBrokenPipeError(err) {
			...
			c.Redirect(http.StatusFound, fmt.Sprintf("/error/%v", http.StatusInternalServerError))
			c.Abort()
		}
		if h.repanic {
			panic(err)
